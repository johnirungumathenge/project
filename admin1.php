...
<div class="container">
    ...
    <?php
    $sql = "SELECT * FROM Request JOIN Users ON Request.users_id=Users.id WHERE Request.approve = ''";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td><form method='POST'><button type='submit' name='submit' class='btn btn-danger p-2' value='" . $row['id'] . "'>Approve</button></form></td>";
            echo "</tr>";
        }
        echo "</table>";
    } else {
        echo "No rows to Review";
    }
...
<div class="container mt-5">
    ...
    <div class="row">
        ...
        <form method="POST" class="form-group">
            ...
            <button type="submit" name="submit" class="btn btn-success mt-2">Post</button>
        </form>
    </div>
</div>
...
if (isset($_POST['submit'])) {
    ...
    $sql = "SELECT * FROM Request WHERE approve = 'approved'";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>alert('No approved requests found.')</script>";
    }
}
